;(function () {
    const Config = {
       snow: ['🕯️', '🗝', '🧙', '🧙‍♂️', '🧙‍♀️', ], // 掉落魔法世界的蜡烛、魔法师、钥匙emoji，每次生成新的emoji时会随机挑选其中一个
        color: '#d9e2e7', // emoji颜色
        speed: 10, // 从生成到落到最底端所经历的时间，单位是秒。数字越小落得越快
        dom: document.getElementsByTagName('body')[0], // 掉落的区域，可以保持不变，这样就是全屏掉落
        interval: 800 // 生成一个emoji的时间间隔，单位是毫秒
    }
    if (!Config.dom) {
        throw Error('错误提示')
    }
